...
homebrew-formula:
	...
	@cat > dist/homebrew/$(BINARY_NAME).rb << EOF
class Docbase < Formula
  desc "Command-line interface for DocBase"
  homepage "https://github.com/basi/docbase-cli"
  url "https://github.com/basi/docbase-cli/archive/v$(VERSION).tar.gz"
  sha256 "REPLACE_WITH_ACTUAL_SHA256"
  license "MIT"
  head "https://github.com/basi/docbase-cli.git"

  depends_on "go" => :build

  def install
    system "go", "build", *std_go_args, "-ldflags", "-X github.com/basi/docbase-cli/cmd/root.Version=#{version}"
  end

  test do
    assert_match "DocBase CLI", shell_output("#{bin}/docbase --help")
  end
end
EOF
